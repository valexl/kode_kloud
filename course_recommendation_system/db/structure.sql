...
CREATE VIEW sequent_schema.command_records AS
 SELECT command.id,
    ...
    command.created_at,
    command.event_aggregate_id,
    command.event_sequence_number
...
CREATE TABLE sequent_schema.saved_event_records (
    ...
    event_json jsonb NOT NULL,
    xact_id bigint,
    CONSTRAINT saved_event_records_operation_check CHECK (((operation)::text = ANY (ARRAY[('U'::character varying)::text, ('D'::character varying)::text])))
);
